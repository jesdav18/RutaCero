...
export class PaymentCalendarPage{
 private readonly http=inject(HttpClient);readonly items=signal<Obligation[]>([]);readonly showForm=signal(false);readonly mobileView=signal<'agenda'|'month'>('agenda');readonly error=signal('');readonly notice=signal('');
 ...
 day(value:string){return value.slice(8,10);}
 month(value:string){return new Intl.DateTimeFormat('es-HN',{month:'short',timeZone:'UTC'}).format(new Date(`${value}T00:00:00Z`)).replace('.','').toUpperCase();}
}
