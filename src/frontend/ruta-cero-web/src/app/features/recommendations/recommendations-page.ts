...
export class RecommendationsPage{private readonly http=inject(HttpClient);readonly items=signal<Recommendation[]>([]);strategy='Avalanche';constructor(){this.load();}load(){this.http.get<Recommendation[]>('/api/v1/recommendations',{params:new HttpParams().set('strategy',this.strategy)}).subscribe(x=>this.items.set(x.map(item=>({...item,confidence:this.confidenceLabel(item.confidence)}))));}private confidenceLabel(value:string){return({High:'Alta',Medium:'Media',Low:'Baja'} as Record<string,string>)[value]??value;}}
