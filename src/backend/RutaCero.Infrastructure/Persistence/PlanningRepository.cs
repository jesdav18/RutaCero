...
public sealed class PlanningRepository(RutaCeroDbContext db):IPlanningRepository
{
 ...
 public Task<ExpectedIncome?> FindIncomeAsync(Guid id,Guid u,CancellationToken t)=>db.ExpectedIncomes.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==u,t);
 public void RemoveIncome(ExpectedIncome x)=>db.ExpectedIncomes.Remove(x);
 ...
 public Task<RecurringCommitment?> FindCommitmentAsync(Guid id,Guid u,CancellationToken t)=>db.RecurringCommitments.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==u&&x.IsActive,t);
